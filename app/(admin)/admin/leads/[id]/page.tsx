...
import { leads, timelineEvents, callLogs, emailLogs, comments, salesExecutives } from "@/lib/mock-data"
import {
  ArrowLeft,
  Phone,
  Mail,
...
export default function AdminLeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const assignedExec = salesExecutives.find(e => e.id === lead.assignedTo)
      ...
      <div className="flex-1 p-4 md:p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm">
            <UserCog className="h-4 w-4 mr-2" /> Reassign Lead
          </Button>
        </div>
                ...
                <div className="flex items-center gap-3">
                  <Avatar className="h-8 w-8">
                    <AvatarFallback className="bg-primary text-primary-foreground text-sm">
                      {assignedExec?.name.split(" ").map(n => n[0]).join("")}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-sm">{assignedExec?.name}</p>
                    <p className="text-xs text-muted-foreground">{assignedExec?.email}</p>
                  </div>
                </div>
              </div>
                        ...
                        <div key={event.id} className="flex gap-3">
                          <div className="relative flex flex-col items-center">
                            <div className={`flex h-8 w-8 items-center justify-center rounded-full ${
                              event.type === "call" ? "bg-success text-success-foreground" :
                              event.type === "email" ? "bg-chart-1 text-white" :
                              event.type === "status_change" ? "bg-warning text-warning-foreground" :
                              "bg-primary text-primary-foreground"
                            }`}>
                              {event.type === "call" ? <Phone className="h-4 w-4" /> :
                               event.type === "email" ? <Mail className="h-4 w-4" /> :
                               event.type === "status_change" ? <Zap className="h-4 w-4" /> :
                               <MessageSquare className="h-4 w-4" />}
                            </div>
                            {index < leadTimeline.length - 1 && (
